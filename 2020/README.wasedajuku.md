...
【課題】
複数の生物を比べてみる。例えば、DNA配列の長さ（[ゲノムサイズ](https://ja.wikipedia.org/wiki/ゲノム#数と大きさ)）、[GC含量](https://ja.wikipedia.org/wiki/GC含量)などを比較する。解析結果をまとめたレポートを提出する。2020-04-19の最終プレゼン大会で発表する研究テーマと参考文献も明記する。
